...
func (c *UserController) RequestToPricefeedRequestOperation(
	...
	for {
		...
		case acknowledgeableBrokerMessage, open := <-sub.MessagesChannel():
			...
			if !open && acknowledgeableBrokerMessage.IsUninitialized() {
				c.logger.Error(ctx, "Channel closed before getting message")
				return PricefeedMessage{}, extensions.ErrSubscriptionCanceled
			}
			...
			// NOTE: it is transformed from the broker again, as it could have
			// been modified by middlewares
			return brokerMessageToPricefeedMessage(acknowledgeableBrokerMessage.BrokerMessage)
		case <-ctx.Done(): // Set corrsponding error if context is done
			c.logger.Error(ctx, "Context done before getting message")
			return PricefeedMessage{}, extensions.ErrContextCanceled
		}
	}
}
...
type PricefeedMessagePayload struct {
	Event     *string                                                `json:"event" validate:"omitempty,eq=history_pricefeed"`
	PriceList []ItemFromPriceListPropertyFromPricefeedMessagePayload `json:"price_list"`
}
